...
class House:
    ...
    def __repr__(self):
        ...
        list_price_str = f" | list_price=${int(self.list_price):,}" if self.list_price != House.DEFAULT_FLOAT else ""
        sold_ppsf_str = f" | sold_ppsf=${int(self.sold_ppsf):,}" if self.sold_ppsf != House.DEFAULT_INT else ""
        list_ppsf_str = f" | list_ppsf=${int(self.list_ppsf):,}" if self.list_ppsf != House.DEFAULT_INT else ""
        ...
        return (f"[addrs={self.address} | price=${int(self.price):,} | bd={int(self.beds)} | "
                ...
                f"lot_size={self.lot_size}{remod_str}{sold_p_str}{list_price_str}{sold_ppsf_str}{list_ppsf_str}{sold_date_str} ]")
